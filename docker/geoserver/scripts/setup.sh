...
array=(geoserver-$GS_VERSION-libjpeg-turbo-plugin.zip geoserver-$GS_VERSION-gdal-plugin.zip)
for i in "${array[@]}"
do
...
if  [[ "$COMMUNITY_MODULES" == true ]]; then
    ...
    array=(geoserver-${GS_VERSION:0:4}-SNAPSHOT-authkey-plugin.zip geoserver-${GS_VERSION:0:4}-SNAPSHOT-sldservice-plugin.zip)
    for i in "${array[@]}"
    do
	    wget --progress=bar:force:noscroll -c --no-check-certificate \
	    https://build.geoserver.org/geoserver/${GS_VERSION:0:5}x/ext-latest/${i} \
	    -O /tmp/resources/plugins/${i}
    done
